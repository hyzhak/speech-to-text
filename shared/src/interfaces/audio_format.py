...
class AudioFormatHandler(ABC):
    ...
    @abstractmethod
    def validate_format(self, file_path: str, expected_format: Optional[str] = None) -> bool:
    ...
    def get_conversion_info(self, source_format: str, target_format: str) -> Dict[str, Any]:
        ...
        if conversion_needed:
            if (source_format.lower() in lossless_formats and target_lossy) or \
               (source_lossy and target_lossy and source_format.lower() != target_format.lower()):
                quality_loss = True
        
        return {
            "needed": conversion_needed,
            "supported": (self.is_format_supported(source_format) and 
                         self.is_format_supported(target_format)),
            "quality_loss": quality_loss,
            "estimated_time": 1.5 if conversion_needed else 0.1  # Time factor multiplier
        }